...
def calcular_pva(pvp):
    ...
    if pd.isna(pvp): return 0.0
    if pvp <= 6.68: return ((pvp - 0.94) / 1.1475) + (0.004 * (pvp / 1.06))
    elif pvp <= 9.97: return ((pvp - 1.95) / 1.1460) + (0.004 * (pvp / 1.06))
    elif pvp <= 14.10: return ((pvp - 2.66) / 1.1436) + (0.004 * (pvp / 1.06))
    elif pvp <= 26.96: return ((pvp - 4.17) / 1.1393) + (0.004 * (pvp / 1.06))
    elif pvp <= 64.68: return ((pvp - 8) / 1.1316) + (0.004 * (pvp / 1.06))
...
def process_data(file_sales, file_master, file_discounts):
    ...
    try:
        ...
        try:
            file_sales.seek(0)
            df_sales = pd.read_csv(file_sales, sep='\t', encoding='latin-1', on_bad_lines='skip', dtype=str)
            if df_sales.shape[1] < 5: raise ValueError("Colunas insuficientes")
        except:
            try:
                file_sales.seek(0)
                df_sales = pd.read_csv(file_sales, sep=';', encoding='latin-1', on_bad_lines='skip', dtype=str)
        ...
        if 'DUV' in df_sales.columns and 'LOCALIZACAO' in df_sales.columns:
            ...
            df_sales['DUV_dt'] = pd.to_datetime(df_sales['DUV'], dayfirst=True, format='mixed', errors='coerce')
            ...
            if not valid_dates.empty:
                ...
                df_sales = df_sales[df_sales['LOCALIZACAO'] == target_loc].copy()
            else:
                st.warning("Aviso: Não foi possível determinar a data mais recente nas vendas. Filtro de localização ignorado.")
        
        # Filtrar por GRUPOHOMOGENEO (se existir)
        # O utilizador mencionou CODIGOHOMOGENEO, vamos verificar ambos
        col_grupo = None
        if 'GRUPOHOMOGENEO' in df_sales.columns: col_grupo = 'GRUPOHOMOGENEO'
        elif 'CODIGOHOMOGENEO' in df_sales.columns: col_grupo = 'CODIGOHOMOGENEO'
        
        if col_grupo:
            df_sales = df_sales[df_sales[col_grupo].notna() & (df_sales[col_grupo].str.strip() != '')]
        ...
        if 'PVP' in df_sales.columns and 'PCU' in df_sales.columns:
            df_sales['MARGEM_REAL_UNIT'] = ((df_sales['PVP'] / 1.06) - df_sales['PCU']).round(2)
        else:
            df_sales['MARGEM_REAL_UNIT'] = 0.0
            
        df_sales['MARGEM_REAL_TOTAL'] = df_sales['MARGEM_REAL_UNIT'] * df_sales['VENDAS_6M']
        
        # Limpeza Chave CPR
        df_sales['CPR'] = df_sales['CPR'].astype(str).str.split('.').str[0].str.strip()
    ...
    try:
        ...
        # Limpeza Chave
        df_master['N_REGISTO'] = pd.to_numeric(df_master['N_REGISTO'], errors='coerce').fillna(0).astype(int).astype(str)
        
        # Filtros Infarmed
        if 'CNPEM' in df_master.columns:
            df_master['CNPEM'] = pd.to_numeric(df_master['CNPEM'], errors='coerce')
            df_master = df_master.dropna(subset=['CNPEM'])
        
        if 'Comerc.' in df_master.columns:
            df_master = df_master[df_master['Comerc.'].astype(str).str.strip() != "Não comercializado"]
            
        # Converter Preços
        for c in ['PVP', 'P_UTENTE', 'P_PENSIONISTA']:
            if c in df_master.columns:
                if df_master[c].dtype == object:
                    df_master[c] = df_master[c].astype(str).str.replace(',', '.', regex=False)
                df_master[c] = pd.to_numeric(df_master[c], errors='coerce').fillna(0.0)
        ...
        # Merge Descontos
        df_master = pd.merge(df_master, df_desc[['CNP', 'DESC']], left_on='N_REGISTO', right_on='CNP', how='left')
        df_master['DESC'] = df_master['DESC'].fillna(0.0)
        
        # Calcular Margem Teórica
        df_master['PVA_LIQ'] = df_master['PVA'] * (1 - df_master['DESC'])
        df_master['MARGEM_TEORICA'] = (df_master['PVP'] / 1.06) - df_master['PVA_LIQ']
    ...
    cols_sales = ['CPR', 'NOM', 'VENDAS_6M', 'MARGEM_REAL_UNIT', 'MARGEM_REAL_TOTAL']
...
def run_abc_analysis(df):
    ...
    if group_col not in df.columns: group_col = 'CNPEM' # Fallback final
...
if up_sales and up_master and up_desc:
    ...
    if df is not None and not isinstance(df, str):
        ...
        class_a['LABEL'] = "#" + (class_a.index + 1).astype(str) + " - " + class_a['NOME_GRUPO'].astype(str) + " (" + class_a['MARGEM_REAL_TOTAL'].apply(lambda x: f"€{x:,.0f}") + ")"
        
        st.subheader("🎯 Grupos Prioritários (Classe A)")
        
        if not class_a.empty:
            sel_label = st.selectbox("Selecione Grupo:", class_a['LABEL'])
            sel_cnpem = class_a[class_a['LABEL'] == sel_label]['CNPEM'].values[0]
            ...
            products = df[df['CNPEM'] == sel_cnpem].copy().sort_values('VENDAS_6M', ascending=False)
            ...
            for _, row in products.iterrows():
                ...
                if not cands.empty:
                    best = cands.sort_values('MARGEM_TEORICA', ascending=False).iloc[0]
            ...
            if results:
                rdf = pd.DataFrame(results).sort_values(['Priority', 'Ganho Est.'], ascending=[True, False])
                
                st.metric("Potencial Ganho Imediato", f"€ {tot_gain:,.2f}")
                st.scatter_chart(rdf, x='Delta Preço', y='Ganho_Visual', color='Ação', size='Vol', height=400)
